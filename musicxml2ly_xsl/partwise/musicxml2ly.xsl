...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="version">musicxml2ly.xsl v0.1.18-1 (22.11.2013)</xsl:variable>
...
\score {
  ...
  &gt;&gt;            
    ...
    <xsl:template match="part">
        ...
        <xsl:choose>
            <!-- 
                Whenever a score contains several parts they are
                usually grouped.  We need to look for part-group
                elements and their group-symbols.  The tricky part
                will be to determine which parts should be grouped
                together with which group-symbol...
                
                LilyPond offers various contexts to group single
                staves together in order to form multi-stave
                systems:
                
                In the StaffGroup context, the group is started
                with a bracket and bar lines are drawn through
                all the staves.  This corresponds to
                <group-symbol>bracket</group-symbol>
                <group-barline>yes</group-barline>
            -->
            <xsl:when test="ancestor::score-partwise/part-list/part-group">
                <xsl:if test="ancestor::score-partwise/part-list/part-group[@type='start']">
                    <xsl:text>
  \new StaffGroup &lt;&lt;
    \set StaffGroup.instrumentName = #"</xsl:text>
                    <xsl:if test="ancestor::score-partwise/part-list/score-part[@id=@id]">
                        <xsl:value-of select="ancestor::score-partwise/part-list/score-part/part-name"/><xsl:text>"</xsl:text>
                    </xsl:if>
                    <xsl:text>            
      \new Staff = "Part</xsl:text><xsl:value-of select="@id"/>
            ...
            <!-- 
                We need to check whether it's a piano score.
                Right now the test is rather unsafe because
                I'm only checking whether a part contains two
                staves.  Can't think of a better way right now.
            -->
    ...
    <xsl:template name="Staff2Voice2">
        ...
        -->
            ...
            <!-- 
                Here we test whether there are more than one
                attributes elements.  If there is only one
                we don't have any problems.  This is just a
                provisional solution because so far we can't
                handle more than one attributes element in
                a measure in any voice other than the first.  
                It's impossible to use a for-each solution here 
                because this would result in all
                attributes elements printed one after another.
                We also have difficulties checking for attributes
                elements in the note template.  We can check if
                an attributes element precedes the current note
                element.  But if we check whether any preceding 
                note element is preceded by an attributes element
                this will hold true for any note element (if the
                measure contains attributes elements).  The attributes
                information would be printed in front of each and
                every note...
            -->
    ...
    <xsl:template match="note">
        ...
        <xsl:choose>
            ...
            <xsl:when test=". and preceding-sibling::*[1][self::attributes] and (.[voice=$voice] or .[staff=$staff])">
                ...
                <xsl:apply-templates select="preceding-sibling::*[1][self::attributes]/clef[@number='1' or not(@number)]">
                    <xsl:with-param name="staff">1</xsl:with-param>
                </xsl:apply-templates>
            </xsl:when>
