...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="version">musicxml2ly.xsl v0.1.15-11 (09.11.2013)</xsl:variable>
    ...
    <xsl:template match="time">
        ...
        \time </xsl:text>
                ...
                <xsl:when test="beats[matches(., '\d+')] 
                    ...
                    <!-- test -->
                    <!--
                    beats: <xsl:value-of select="count(beats)"/>
                    beat-type: <xsl:value-of select="count(beat-type)"/>
                    -->
                </xsl:when>
                <!-- senza misura -->
