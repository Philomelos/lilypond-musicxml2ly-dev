...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="version">musicxml2ly.xsl v0.1.14 (18.10.2013)</xsl:variable>
...
\score {
  ...
  &gt;&gt;            
    ...
    \new Staff = "Part</xsl:text><xsl:value-of select="@id"/>
                <xsl:text>Staff1" &lt;&lt;</xsl:text>        
                <xsl:if test="/score-partwise/part/measure/note[staff='1' or voice='1']">
                    <xsl:text>
    ...
    <xsl:template match="alter">
        ...
        <xsl:choose>
            <xsl:when test=".">
                <xsl:value-of select="$alter/entry[@key=$semitones]"/>
            </xsl:when>
        </xsl:choose>
    ...
    <xsl:template match="type">
        ...
        <!--
                        ...
                        -->
        <xsl:choose>
        <xsl:when test="rest/display-step and .='128th'"><xsl:value-of select="$durations/entry[@key='128th']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='64th'"><xsl:value-of select="$durations/entry[@key='64th']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='32nd'"><xsl:value-of select="$durations/entry[@key='32nd']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='16th'"><xsl:value-of select="$durations/entry[@key='16th']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='eighth'"><xsl:value-of select="$durations/entry[@key='eighth']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='quarter'"><xsl:value-of select="$durations/entry[@key='quarter']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='half'"><xsl:value-of select="$durations/entry[@key='half']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='whole'"><xsl:value-of select="$durations/entry[@key='whole']"/>\rest</xsl:when>
        <xsl:when test="rest/display-step and .='breve'"><xsl:value-of select="$durations/entry[@key='breve']"/>\rest</xsl:when>
        <xsl:when test=".='128th'"><xsl:value-of select="$durations/entry[@key='128th']"/></xsl:when>
        <xsl:when test=".='64th'"><xsl:value-of select="$durations/entry[@key='64th']"/></xsl:when>
        <xsl:when test=".='32nd'"><xsl:value-of select="$durations/entry[@key='32nd']"/></xsl:when>
        <xsl:when test=".='16th'"><xsl:value-of select="$durations/entry[@key='16th']"/></xsl:when>
        <xsl:when test=".='eighth'"><xsl:value-of select="$durations/entry[@key='eighth']"/></xsl:when>
        <xsl:when test=".='quarter'"><xsl:value-of select="$durations/entry[@key='quarter']"/></xsl:when>
        <xsl:when test=".='half'"><xsl:value-of select="$durations/entry[@key='half']"/></xsl:when>
        <xsl:when test=".='whole'"><xsl:value-of select="$durations/entry[@key='whole']"/></xsl:when>
        <xsl:when test=".='breve'"><xsl:value-of select="$durations/entry[@key='breve']"/></xsl:when>
