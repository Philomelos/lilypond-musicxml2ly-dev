...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- musicxml2ly.xsl v0.1.12 (15.10.2013) -->
    <!-- Dictionaries -->
    <!-- octave dictionary -->
...
\score {
  &lt;&lt;</xsl:text>
        ...
        <xsl:apply-templates select="greatest-common-divisor"/>
        <xsl:apply-templates select="decimal-to-fraction"/>
        -->
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template match="score-partwise">
        <xsl:for-each select="part">
            <!-- Here we need a check for several staves, partgroups and voices -->
            <!-- TEST -->
            <xsl:text>
                %%%%%%%%% TEST %%%%%%%</xsl:text>
            <!--
            <xsl:choose>
                <xsl:when test="/score-partwise/part/measure/attributes/staves='2' and /score-partwise/part/measure/note/staff">
    \new PianoStaff &lt;&lt;
                </xsl:when>
                <xsl:when test="/score-partwise/part/measure/attributes/staves='2' and /score-partwise/part/measure/note/staff='1'">
                    \new Staff &lt;&lt;
                </xsl:when>
            </xsl:choose>    
            -->
    ...
    \new Staff = "Part<xsl:value-of select="@id"/><xsl:text>Staff1" \with {
      ...
      \new Voice = "Part<xsl:value-of select="@id"/><xsl:text>Staff1Voice1" {
            ...
            <xsl:for-each select="measure">
                ...
                <!-- Test -->
                <!--
                <xsl:text>pickup measure? </xsl:text>
                <xsl:choose>
                ...
                <!-- and 
                     ...
                     <xsl:value-of select="sum(/score-partwise/part/measure/note/duration)"/>
                -->
                ...
                <xsl:for-each select="attributes">
                    ...
                    -->
                        <!-- 
                            Save the value of 4 divided by <divisions> in variable divisions.
                            This variable is needed to compute the LilyPond note durations by
                            multiplying divisions by durations.
                        -->
                        <!--
                        <xsl:variable name="divisions" select="4 div /score-partwise/part/measure/attributes/divisions" />
                        -->
                    ...
                    </xsl:for-each> <!-- end measure style -->
                <!-- 
                ...
                -->
                    ...
                    <!-- Test -->
                    <!--
                        <xsl:value-of select="(sum(/score-partwise/part/measure[@number='0']/note/duration) div 4)"/>
                        
                        (sum(/score-partwise/part/measure[@number='0']/note/duration) div 4)"/>
                        and 
                            /score-partwise/part/measure[@implicit='yes'] 
                            and 
                            (sum(/score-partwise/part/measure/note/duration) div 4) 
                            lt 
                            (/score-partwise/part/measure/attributes/divisions 
                            * 
                            /score-partwise/part/measure/attributes/time/beats 
                            div 
                            /score-partwise/part/measure/attributes/time/beat-type)
                    <xsl:value-of select="(sum(/score-partwise/part/measure/note/duration) div 4) 
                        lt 
                        (/score-partwise/part/measure/attributes/divisions 
                        * 
                        /score-partwise/part/measure/attributes/time/beats 
                ...
                <xsl:for-each select="note">
                    <!-- Is the following line still needed?  No! -->
                    <!--
                    <xsl:variable name="duration" select="/score-partwise/part/measure/note" />
                    -->
                    ...
                    <xsl:for-each select="pitch">
                        ...
                        -->
                        <!-- Link to alter dictionary in the following line does not work! -->
                        <!--
                    ...
                    <xsl:choose>
                        ...
                        <!-- Check whether there is a duration element.
                            ...
                            4/divisions * duration
                        -->
                        <xsl:when test="duration and not(type)">
                            <xsl:call-template name="decimal-to-fraction">
...
%  short-indent = 0.5\cm
        ...
        <!--
        <xsl:for-each select="/score-partwise/part/measure/note/rest">
            <xsl:variable name="quartervalue" select="4" />
            <xsl:variable name="divisions" select="/score-partwise/part/measure/attributes/divisions" />
            <xsl:variable name="duration" select="/score-partwise/part/measure/note/duration" />
            <xsl:value-of select="$quartervalue div $divisions"/>
            <xsl:text>
                
            </xsl:text>
            <xsl:for-each select="/score-partwise/part/measure/note/duration">
                <xsl:variable name="duration" select="/score-partwise/part/measure/note/duration" />
                bla
