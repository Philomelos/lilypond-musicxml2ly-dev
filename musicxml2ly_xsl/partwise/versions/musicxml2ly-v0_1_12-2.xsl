...
\score {
  ...
  &gt;&gt;            
    ...
    <xsl:template name="staves">
        <!-- 
            Check if there is a <staves> element.  If yes: print
            \new PianoStaff << ...
            (It could also be a ChoirStaff!)
            print x \new Staff << blocks (x=value of <staves>)
            If there is no <staves> element: print
            \new Staff << ...
        -->
        <!--
        <xsl:variable name="partid" select="@id"></xsl:variable>
        -->
        <!--
        <xsl:choose>
            <xsl:when test="/score-partwise/part/measure[1]/attributes/staves">
                <xsl:text>\new PianoStaff &lt;&lt;
      \set PianoStaff.instrumentName = #"</xsl:text><xsl:value-of select="/score-partwise/part-list/score-part/part-name"/><xsl:text>"
      </xsl:text>
                <xsl:variable name="x" select="0"></xsl:variable>
                <xsl:variable name="staves" select="/score-partwise/part/measure[1]/attributes/staves"/>
                <xsl:for-each select="for $x in //staves return $x+1"><xsl:text>
                    \new Staff = "Part</xsl:text><xsl:value-of select="//@id"/><xsl:text>Staff</xsl:text><xsl:value-of select="$x+1"/><xsl:text>" &lt;&lt;
                </xsl:text>
                </xsl:for-each>
            </xsl:when>
        </xsl:choose>
        ...
        <!--
        <xsl:for-each select="for $x in //staves return 'new Staff'">\new Staff</xsl:for-each>
        -->
